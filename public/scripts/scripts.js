if ($) console.log('scripts made it and jquery works . . .')
if (!$) console.log('scripts made it and jquery is broken . . .')

// ----------------- MENU / NAV SCRIPTS -----------------


$('.hamburger-menu-button').on('click', (e) => handleHamburgerMenuClick(e))
$('.nav-button').each(function() {
  $(this).on('click', (e) => handleMenuItemClick(e, this))
})

const handleHamburgerMenuClick = (e) => {
  $('.main-menu-nav').toggleClass('hidden')
  $('.hamburger-menu-button').toggleClass('button-open')
}

const handleMenuItemClick = (e, buttonClicked) => {
  const position = $(`.${$(buttonClicked).attr('class').split(' ')[0].split('-')[0]}`).offset()
  const tenVH = $(window).height() * .1
  $('.main-menu-nav').toggleClass('hidden')
  $('.hamburger-menu-button').toggleClass('button-open')
  $('html').animate({ scrollTop: (position.top - tenVH) }, 'slow')
}

// ----------------- END MENU / NAV SCRIPTS -----------------
